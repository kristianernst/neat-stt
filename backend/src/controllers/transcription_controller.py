...
class TranscriptionController:
  ...
  async def transcribe_audio(
    ...
    try:
      ...
      async def event_generator():
        try:
          yield {"event": "ready", "data": json.dumps({"status": "ready"})}
          ...
          transcription_task = asyncio.create_task(asyncio.to_thread(lambda: asyncio.run(process_transcription())))
    ...
    try:
      ...
      async def event_generator() -> AsyncGenerator[Dict[str, Any], None]:
        try:
          ...
          transcription_gen = self.stt.transcribe_live(chunk_duration_ms=chunk_duration_ms, save_debug=save_debug)
          ...
          for result in transcription_gen:
            if result and result.get("text", "").strip():
              ...
              yield {"event": "transcription", "data": json.dumps(result)}
  ...
  async def stop_live_transcribe(self) -> Dict[str, str]:
    ...
    try:
      ...
      try:
        await asyncio.wait_for(asyncio.to_thread(stop_with_timeout), timeout=stop_timeout)
